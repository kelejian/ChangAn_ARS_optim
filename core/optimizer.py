...
def summarize_optimization(
    ...
    typical = high_speed.sort_values(
        by=["RecommendationRank", "Reduction_MAIS", "Reduction_MAIS_expected"],
...
def _score_candidates_batch(
    ...
    分数越低表示候选越优；其中 expected_mais 和 p_mais_ge3 来自连续损伤指标经 AIS 映射得到的 MAIS 结果，uncertainty 和 ood 表示预测可信度惩罚，action_cost 表示相对原始控制参数的调整幅度。
    """
    action_cost = _action_cost_batch(candidates, base_cases)
    score = (
        float(weights.get("expected_mais", 1.0)) * pred_df["pred_MAIS_expected"]
...
def _build_result_row(
    ...
    row["Base_MAIS_expected"] = float(base_pred["pred_MAIS_expected"])
    row["Opt_MAIS_expected"] = float(best_pred["pred_MAIS_expected"])
    row["Reduction_MAIS_expected"] = row["Base_MAIS_expected"] - row["Opt_MAIS_expected"]
...
def _recommendation_flag(row: Dict[str, object]) -> Tuple[str, int]:
    """依据预测降损和置信度给出推荐标志。"""
    improves = float(row["Reduction_MAIS_expected"]) > 0.05 or int(row["Reduction_MAIS"]) >= 1
...
def _subset_summary(df: pd.DataFrame, name: str) -> Dict[str, object]:
    ...
    if len(df) == 0:
        return {
            ...
            "mean_reduction_mais_expected": None,
    ...
    return {
        ...
        "mean_reduction_mais_expected": float(df["Reduction_MAIS_expected"].mean()),
        ...
        "mean_base_mais_expected": float(df["Base_MAIS_expected"].mean()),
        "mean_opt_mais_expected": float(df["Opt_MAIS_expected"].mean()),
