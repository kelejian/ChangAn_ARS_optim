...
本模块依据 reference_doc/4_Injury_Criteria_AIS_Cal.py 中的计算逻辑实现。代理模型直接预测 Amax、Dmax、HIC 和 Nij，CTI 由 Amax、Dmax 与乘员体型参数按参考公式派生，AIS 与 MAIS 再由 HIC、CTI、Nij 映射得到。
"""
...
def append_ais_from_injury_metrics(pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    result["pred_MAIS_expected"] = result["pred_MAIS"].astype(float)
